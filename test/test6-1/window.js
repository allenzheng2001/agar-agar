...
class Window 
{
    ...
    show(cur_pov, zoom)
    {
        ...
        for(let object of this.show_buf)
            object.show();
        this.show_buf = [];
    }
...
class Frame
{
    ...
    constructor(x, y, edge)
    {
        ...
        for(var i = 0; i < this.biome.food_density; i++)
    ...
    show()
    {
        this.biome.show(this.minX, this.minY, init_dim);
        for (var i = this.food.length - 1; i >= 0; i--) {
            world.show_buf.push(this.food[i]);
        }
        for(let hazard of this.hazards)
            world.show_buf.push(hazard);
    }
